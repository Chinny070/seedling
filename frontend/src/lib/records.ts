...
export interface LatentSet { candidate_id:string; frozen:boolean; candidate_status:string; evidence_count:number; distinct_category_count:number; distinct_categories:string[]; distinct_host_count:number; distinct_hosts:string[]; minimum_evidence_categories:number; minimum_independent_sources:number; requirements_met:boolean }
...
export const toEpoch = (value:string) => Math.floor(Date.parse(value + "T00:00:00Z") / 1000);
